...
export default function generator(plop: PlopTypes.NodePlopAPI): void {
  ...
  const workspaceNameArray = Object.values(WorkspaceName).filter((m) =>
    isNaN(Number(m))
  ...
  plop.setGenerator("react-component", {
    ...
    prompts: [
      ...
      },
      {
        ...
        message: (data) =>
          `Choose ${(data as InquirerDataType).workspaceName} name: `,
        choices: (data) =>
          getDirectories((data as InquirerDataType).workspaceName),
      },
    ],
  ...
  function getNestedRouteActions(data: InquirerDataType) {
    ...
    const specificDir =
      workspaceName.toLowerCase() === "apps"
        ? `${appOrPackageName}/app/components/`
        : `${appOrPackageName}/src/`;
